...
const TopMenuSelect: React.FC = () => {
    ...
    const handleChange = (e: SelectChangeEvent) => {
        ...
        if (!path.startsWith("/check") && window.location.pathname !== value) removeFilter()
        navigate(value)
    }
    ...
    return (
        ...
        >
            {getOptions()!.map((menu) => <MenuItem key={menu.subpath} value={`/${category}/${menu.subpath}`}>{t(menu.labelTag)}</MenuItem>)}
        </Select>
    )
}
