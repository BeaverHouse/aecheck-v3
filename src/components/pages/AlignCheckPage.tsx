...
const CharacterStaralign = lazy(() => import("../molecules/CharacterStaralign"));
...
function AlignCheckPage() {
    ...
    const baseCharacters = characters.filter((c) => c.tags.includes("staralign.true"))
        .sort((a, b) => {
    ...
    return (
        <Box sx={{
            ...
            <Filterbox type="STARALIGN" filteredInfo={filteredArr} label={t("frontend.search.char")} />
            <Suspense fallback={<CircularProgress sx={{ margin: 10 }} />}>
                <Box sx={{
                    ...
                    {filteredArr.map((c) => <CharacterStaralign key={c.id} {...c} />)}
                </Box>
            </Suspense>
        </Box>
    )
