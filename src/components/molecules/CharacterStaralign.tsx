...
function CircularProgressWithLabel(
    ...
    return (
        <Box sx={{ position: 'relative', display: 'inline-flex', mr: 2 }}>
            <CircularProgress variant="determinate" color='secondary' size={60} {...props} />
            <Box
                sx={{
                    top: 0,
                    left: 0,
...
const CharacterStaralign: React.FC<CharacterInfo> = (info) => {
    ...
    const currentStep = getAlignStep(info, staralign)
            ...
            {inven.includes(info.id)
                ? <Box sx={{ display: 'flex', alignItems: "center", pl: 1, pr: 1 }}>
                    ...
                    <Box>
                        ...
                        <Typography fontWeight={600}>{`${currentStep} / ${maxStep}`}</Typography>
                    </Box>
                </Box>
                : <Typography sx={{ ml: 2, display: "flex", alignItems: "center" }} variant='subtitle2'>{status}</Typography>}
        </Card >
    )
}
